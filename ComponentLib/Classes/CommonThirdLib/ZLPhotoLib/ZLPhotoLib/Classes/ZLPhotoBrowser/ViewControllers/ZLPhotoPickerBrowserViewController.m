...
@property (assign,nonatomic) NSInteger isHiddenPath;
@property (assign,nonatomic) BOOL zeroIsHidden;
...
- (NSArray *)getPhotos{
    ...
    for (ZLPhotoPickerBrowserPhoto *photo in _photos) {
        if (photo.hasOriginPhotoInCache) {
            photo.photoURL = photo.originPhotoURL;
        }
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (self.photos.count) {
        ...
        if (photo.hasOriginPhotoInCache && photo.photoURL != photo.originPhotoURL) {
            photo.photoURL = photo.originPhotoURL;
        }
        ...
        originalBtn.tag = indexPath.item + 200;
        originalBtn.frame = CGRectMake(100, scrollView.frame.size.height - 50 - 50, self.view.frame.size.width - 200, 50);
        
        originalBtn.titleLabel.font = [UIFont systemFontOfSize:16];
        originalBtn.titleLabel.textAlignment = NSTextAlignmentCenter;
        [originalBtn setTitleColor:[UIColor lightGrayColor] forState:UIControlStateNormal];
        originalBtn.layer.cornerRadius = 2;
        originalBtn.layer.masksToBounds = YES;
        originalBtn.hidden = YES;
        [originalBtn setTitle:@"查看原图" forState:UIControlStateNormal];
        [originalBtn addTarget:self action:@selector(btnClick:) forControlEvents:UIControlEventTouchUpInside];
        
        [scrollBoxView addSubview:originalBtn];

        if (photo.hasOriginPhotoInCache) {
            originalBtn.hidden = YES;
        }
        else {
            if (![photo.photoSize isEqualToString:@""]) {
                originalBtn.hidden = NO;
                [originalBtn setTitle:[NSString stringWithFormat:@"查看原图(%@)",photo.photoSize] forState:UIControlStateNormal];
            }
            else {
                [photo requestOriginImageSize:photo.originPhotoURL.absoluteString success:^(NSInteger index){
                
                    [weakSelf hideOriginBtn:NO withIndex:index];
                
                    [weakSelf.collectionView reloadItemsAtIndexPaths:@[[NSIndexPath indexPathForRow:weakSelf.currentPage inSection:0]]];
                } fail:^(NSInteger index){
                    [weakSelf hideOriginBtn:YES withIndex:index];
                    originalBtn.hidden = YES;
                    [weakSelf.collectionView reloadItemsAtIndexPaths:@[[NSIndexPath indexPathForRow:weakSelf.currentPage inSection:0]]];
                }];
            }
...
- (void)hideOriginBtn:(BOOL)isHide withIndex:(NSInteger)index {
    if (index == _currentPage) {
        UICollectionViewCell *currCell = [self.collectionView cellForItemAtIndexPath:[NSIndexPath indexPathForItem:index inSection:0]];
        UIButton *btn = [currCell viewWithTag:index+200];
        ZLPhotoPickerBrowserPhoto *currPhoto = self.photos[index];
        if (btn) {
            btn.hidden = isHide;
            [btn setTitle:[NSString stringWithFormat:@"查看原图(%@)",currPhoto.photoSize] forState:UIControlStateNormal];
        }
    }
}
...
//    photo.photoURL = self.originalArr[_currentPage];
//
//    NSMutableArray * temp = [NSMutableArray arrayWithArray:self.photos];
//    [temp replaceObjectAtIndex:_currentPage withObject:photo];
//
//    self.photos = temp;
//    if (sender.tag - 200 == 0) {
//        _zeroIsHidden = YES;
//    } else {
////        _zeroIsHidden = NO;
//        _isHiddenPath = sender.tag-200;
//    }
    __weak typeof(self)weakSelf = self;
    NSUInteger current = sender.tag - 200;
    ZLPhotoPickerBrowserPhoto *photo = self.photos[current];
    photo.photoURL = photo.originPhotoURL;
    
    [[SDWebImageManager sharedManager] loadImageWithURL:photo.photoURL options:0 progress:^(NSInteger receivedSize, NSInteger expectedSize, NSURL * _Nullable targetURL) {
        if (current == weakSelf.currentPage) {
            dispatch_async(dispatch_get_main_queue(), ^{
                UICollectionViewCell *currCell = [weakSelf.collectionView cellForItemAtIndexPath:[NSIndexPath indexPathForItem:current inSection:0]];
                UIButton *btn = [currCell viewWithTag:current+200];
                if (btn) {
                    int percent = (int)receivedSize*100/(int)expectedSize;
                    if (percent >= 100) {
                        btn.hidden = YES;
                    }
                    else {
                        [btn setTitle:[NSString stringWithFormat:@"查看原图(%d%%)",percent] forState:UIControlStateNormal];
                    }
                }
            });
        }
        ...
        ZLPhotoPickerBrowserPhoto *photo = self.photos[current];
        photo.hasOriginPhotoInCache = YES;
        [weakSelf.collectionView reloadItemsAtIndexPaths:@[[NSIndexPath indexPathForRow:current inSection:0]]];
    }];
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView{
    ...
    [self.collectionView reloadItemsAtIndexPaths:@[[NSIndexPath indexPathForItem:currentPage inSection:0]]];
}
