...
static YYOUCarUserInfoManager *userInfo;

@implementation YYOUCarUserInfoManager
...
+(instancetype)shareUserInfo{
    ...
    dispatch_once(&once, ^{
        userInfo = [[YYOUCarUserInfoManager alloc] init];
    });
    return userInfo;
}
+(void)configInfo:(NSDictionary *)infoDict{
    ...
    for (NSString *key in allKeys) {
        ...
        //调用setter方法
        NSString *value = [NSString nullToString:[infoDict objectForKey:key]];
        YYOUCarUserInfoManager *manager = [YYOUCarUserInfoManager shareUserInfo];
